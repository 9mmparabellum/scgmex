...
import { useConceptosIngreso, useResumenIngresos } from '../../hooks/usePresupuesto';
import { MOMENTOS_INGRESO } from '../../config/constants';
...
export default function PresupuestoIngresos() {
  ...
  const { data: resumen = {}, isLoading: loadingResumen } = useResumenIngresos();
  const { data: conceptos = [], isLoading: loadingConceptos } = useConceptosIngreso();
  ...
  // --- Totals per momento across all conceptos ---
  const totalesMomento = useMemo(() => {
    const totals = {};
    MOMENTOS_INGRESO.forEach((m) => {
      totals[m.key] = 0;
    });
    conceptos.forEach((c) => {
      MOMENTOS_INGRESO.forEach((m) => {
        totals[m.key] += Number(c.totales?.[m.key] || 0);
      });
    });
    return totals;
  }, [conceptos]);
  ...
  const handleExport = () => {
    ...
    exportToExcel(conceptos, excelCols, 'presupuesto_ingresos');
  };

  const isLoading = loadingResumen || loadingConceptos;

  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-bold text-text-primary">Presupuesto de Ingresos</h1>
        <p className="text-sm text-text-muted mt-1">
          Resumen del presupuesto de ingresos por momento contable
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-16 text-text-muted text-sm">
          ...
          Cargando presupuesto de ingresos...
        </div>
      ) : (
        <>
          {/* Summary cards */}
          ...
          <div className="bg-white rounded-lg card-shadow p-5">
            ...
            {conceptos.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-text-muted">
                ...
                <p className="text-[0.9375rem]">No hay conceptos registrados</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {conceptos.map((concepto, idx) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-[0.9375rem] text-text-primary">
                          {concepto.clasificador
                            ? `${concepto.clasificador.codigo} ${concepto.clasificador.nombre}`
                            : '\u2014'}
                        </td>
                        {MOMENTOS_INGRESO.map((m) => (
                          <td
                          ...
                          >
                            {fmtCurrency(concepto.totales?.[m.key])}
                          </td>
                        ))}
                      </tr>
                    ))}

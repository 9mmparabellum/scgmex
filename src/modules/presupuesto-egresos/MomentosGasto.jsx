...
export default function MomentosGasto() {
  const { entePublico, ejercicioFiscal } = useAppStore();

  const [selectedMomento, setSelectedMomento] = useState(MOMENTOS_GASTO[0].key);
  ...
  const queryFilter = useMemo(() => {
    ...
    if (filterPartida) f.partida_id = filterPartida;
  ...
  const createMut = useCreate('movimiento_presupuestal_egreso');
  const removeMut = useRemove('movimiento_presupuestal_egreso');

  // --- Lookup maps ---
  const partidaMap = useMemo(() => {
    const map = {};
    partidas.forEach((p) => { map[p.id] = p; });
    return map;
  }, [partidas]);

  // --- Select options ---
  const partidaOptions = useMemo(
    () => partidas.map((p) => ({ value: p.id, label: `${p.clave} — ${p.descripcion}` })),
    [partidas]
  );
  ...
  const momentoLabel = MOMENTOS_GASTO.find((m) => m.key === selectedMomento)?.label ?? selectedMomento;
  ...
  const handleExport = () => {
    const excelCols = [
      ...
      },
      {
        ...
        label: 'Partida',
        getValue: (row) => {
          const p = partidaMap[row.partida_id];
          return p ? `${p.clave} — ${p.descripcion}` : row.partida_id;
        },
      },
      {
        key: 'tipo_movimiento',
    ...
    exportToExcel(movimientos, excelCols, `momentos_gasto_${selectedMomento}`);
  };
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-bold text-text-primary">Momentos del Gasto</h1>
        <p className="text-sm text-text-muted mt-1">
          Registro de movimientos presupuestarios de egreso
        </p>
      </div>
      ...
      >
        <div className="space-y-4">
          ...
          <Select
            ...
            onChange={(e) => handleChange('partida_id', e.target.value)}
            options={partidaOptions}
          ...
          <div className="flex justify-end gap-3 pt-2 border-t border-border">
            ...
            <Button
              onClick={handleSave}
              loading={isSaving}
              disabled={!form.partida_id || !form.tipo_movimiento || !form.monto || !form.fecha}
            >
              Registrar movimiento
            </Button>
          </div>
